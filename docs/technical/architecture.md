...
They are untrusted conversational context, never current evidence or
instructions. Direct recall cites `(turn_index, role)` messages. When the recent
window is insufficient, the internal `conversation.history_lookup` tool may
retrieve older messages once; the result is request-local context and the
Controller is re-entered without turning history into Dota evidence. A
`session_id` remains a bearer capability for one user security subject, so
cross-session history access is unavailable.
