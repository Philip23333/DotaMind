...
强化图标保留在括号内。普通比赛详情不展示加点或天赋。存在 `player_match_progress` evidence 时，
Answer 额外获得按需的“出装、加点与天赋”Markdown 章节规则：只有当前问题明确要求这些历史
进度事实时才展开目标选手的完整配置包（出门装、最终装备、确定性出装路径、技能加点和天赋选择）。
购买路径由 progress transform 的 `purchase_display` 投影提供：负时间事件按购买顺序平铺为出门装，
非负时间事件按维护的消耗品/守卫内部名集合过滤，且只为终件或维护的关键中间装备附里程碑时间；
Answer 只输出以 `→` 连接的单行证据名称，不重新按价格或名称筛选，也不输出购买顺序表格或手写图片 URL；
Chat 将该具名选手进度章节的出门装替换为大尺寸、仅图标的 Catalog 展示；最终装备同样只显示图标，
其中主栏使用大尺寸，背包、中立与强化使用中尺寸。出装路径保持中尺寸图标和名称；技能加点箭头序列
按原始升级顺序保留每个普通技能和全属性加成、剔除天赋，并在其中
