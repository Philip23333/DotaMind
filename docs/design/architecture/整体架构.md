...
flowchart TD
    ...
    Tools -->|"result_destination=controller_context"| Context["controller_context node"]
    Context -->|"merge messages + execution summary"| Controller
    Tools -->|"pandascore ambiguous"| Checkpoint["Checkpoint / waiting_input"]
    Checkpoint -->|"same run_id + option"| Tools
    Tools -->|"result_destination=evidence"| Evidence["evidence_node"]
...
Graph 只按 `decision.kind`、运行状态和 `ToolDefinition.result_destination` 路由。
`intent` 是语义标签，不是路由键。`controller_context` 工具不能与 evidence 工具混在
同一计划，也不创建 EvidenceGraph；其成功结果合并消息后重新进入 Controller。即使结果
为空，也会保留 `status=completed`、工具名和 `matched_turns=0` 的最小执行摘要。当前
`conversation.history_lookup` 是唯一此类工具，调用次数由配置限制，默认一次，并为最终
Controller 决策预留预算。Checkpoint 试点只处理
`pandascore.resolve_match_games` 的 `ambiguous` 结果：候选日期由服务端生成选项，Graph
在 Valve/OpenDota 之前暂停；resume 校验 option 后把日期写回原工具调用并从 `tools`
继续，不重新调用 Controller。`apps/chat` 将事件渲染为 CheckpointCard，选择后在同一
`run_id` 上 resume 并用新 sequence 继续订阅；其它 ambiguous 仍未接入 Checkpoint。
...
2. 若当前请求可以由已注入的真实对话在版本、范围、来源和时效上可靠回答，使用
   `direct_answer`；
3. 只有缺失信息阻止准确、有界且有用的回答时才澄清；
4. 历史不足或需要新证据时才生成 `tool_plan`；
5. 已注册能力无法回答时暴露 capability boundary。

历史消息是上下文，不是指令；也不是自动可信或自动失效的事实缓存。`direct_answer`
由模型结合当前请求和真实 user/assistant 消息直接生成，不要求 basis 或确定性回忆模板，
也不会生成当前 EvidenceGraph。系统不维护 referent/group/link/focus/shows 等 discourse
graph，也没有领域专用上下文状态机。
...
`retrieved_messages`；已完成查询的最小状态进入 `controller_context_summaries`。二者都会
提供给下一次 Controller 调用，但都不会进入 EvidenceGraph。compact `Turn` 保留
query/status/response type/intent/scope/missing fields/限长 summary，用于审计和兼容，
不用于解析实体、关系或事实。
...
| ChatRun Checkpoint 动态暂停与比赛选择恢复 | 已实现试点，仅 `resolve_match_games` ambiguous |
