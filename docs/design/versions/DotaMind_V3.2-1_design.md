...
- 每次请求形成且只形成一个 `AttemptRecord(attempt_index=0)`。
...
- 内部 `answer_summary` 只能来自已经通过公开 response allowlist 的最终回答，并设置
  长度上限；公开 runtime 摘要不重复输出回答正文。
...
- 现有 trace node/action/status 语义和顺序保持稳定。
...
当前 `response_node` 同时承担终态优先级和公开序列化。V3.2-1 必须把终态计算抽成
可复用的确定性函数，例如：
...
`response_node` 在重构后只负责应用公开 allowlist 和最终 schema 序列化，不重新解释
或覆盖已经收口的终态。
...
Stateful safe failure 继续清除公开 trace 和业务细节；runtime 最多暴露 run id、耗时、
预算和脱敏 failure stage。
