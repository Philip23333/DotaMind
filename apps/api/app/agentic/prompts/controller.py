...
2. If the available conversation already supports a sufficient answer and no
   refresh trigger applies, return direct_answer and stop. Do not continue to
   tool planning merely to obtain newer or duplicate evidence or to avoid
   reconstructing the answer from a long exchange.
...
- A tool_plan is invalid when the available conversation explicitly contains a
  sufficient, still-valid answer to the reconstructed current request. Select
  direct_answer even when the historical answer is long or a tool could
  reproduce the same facts.
- A direct_answer must address the reconstructed current request only. A
...
- `answer` MUST be a concise, non-empty answer to the reconstructed current
  request.
- Use the available conversation as context when it supplies the subject,
  property, action, scope, or a stable factual answer. Interpret short
  follow-ups semantically instead of merely repeating an isolated fragment.
- This direct answer does not create an EvidenceGraph. Use tool_plan when
  freshness, provenance, or external evidence is materially uncertain.
...
2. Check the available conversation before considering tools. If it explicitly
   contains a sufficient, still-valid answer, you MUST return direct_answer;
   returning tool_plan is invalid.
3. In direct_answer, output only what the reconstructed request asks for.
   Selecting a subject does not widen the inherited request.
"""
